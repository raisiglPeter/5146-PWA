let e;var t,n,s,o,i,a,r,l,c,d,u,h,f,p,g,m,E,y,C,v,O,I,_,T,b=globalThis,w={},S={},R=b.parcelRequire94c2;null==R&&((R=function(e){if(e in w)return w[e].exports;if(e in S){var t=S[e];delete S[e];var n={id:e,exports:{}};return w[e]=n,t.call(n.exports,n,n.exports),n.exports}var s=Error("Cannot find module '"+e+"'");throw s.code="MODULE_NOT_FOUND",s}).register=function(e,t){S[e]=t},b.parcelRequire94c2=R),R.register,(t=f||(f={})).STRING="string",t.NUMBER="number",t.INTEGER="integer",t.BOOLEAN="boolean",t.ARRAY="array",t.OBJECT="object",(n=p||(p={})).LANGUAGE_UNSPECIFIED="language_unspecified",n.PYTHON="python",(s=g||(g={})).OUTCOME_UNSPECIFIED="outcome_unspecified",s.OUTCOME_OK="outcome_ok",s.OUTCOME_FAILED="outcome_failed",s.OUTCOME_DEADLINE_EXCEEDED="outcome_deadline_exceeded";/**
 * @license
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */async function es(e,t,n,s){return(await $(t,T.COUNT_TOKENS,e,!1,JSON.stringify(n),s)).json()}/**
 * @license
 * Copyright 2024 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */async function eo(e,t,n,s){return(await $(t,T.EMBED_CONTENT,e,!1,JSON.stringify(n),s)).json()}async function ei(e,t,n,s){let o=n.requests.map(e=>Object.assign(Object.assign({},e),{model:t}));return(await $(t,T.BATCH_EMBED_CONTENTS,e,!1,JSON.stringify({requests:o}),s)).json()}/**
 * @license
 * Copyright 2024 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 */class er{constructor(e){this.apiKey=e}getGenerativeModel(e,t){if(!e.model)throw new N("Must provide a model name. Example: genai.getGenerativeModel({ model: 'my-model-name' })");return new ea(this.apiKey,e,t)}getGenerativeModelFromCachedContent(e,t,n){if(!e.name)throw new x("Cached content must contain a `name` field.");if(!e.model)throw new x("Cached content must contain a `model` field.");for(let n of["model","systemInstruction"])if((null==t?void 0:t[n])&&e[n]&&(null==t?void 0:t[n])!==e[n]){if("model"===n&&(t.model.startsWith("models/")?t.model.replace("models/",""):t.model)===(e.model.startsWith("models/")?e.model.replace("models/",""):e.model))continue;throw new x(`Different value for "${n}" specified in modelParams (${t[n]}) and cachedContent (${e[n]})`)}let s=Object.assign(Object.assign({},t),{model:e.model,tools:e.tools,toolConfig:e.toolConfig,systemInstruction:e.systemInstruction,cachedContent:e});return new ea(this.apiKey,s,n)}}var el=R("47Mwn");const ec=document.getElementById("recipe-steps"),ed=document.querySelector(".recipe-step-button"),eu=document.querySelector(".steps-preview ol"),eh=document.getElementById("recipe-tags"),ef=document.querySelector(".recipe-tag-button"),ep=document.querySelector(".tags-preview p"),eg=document.getElementById("add-modal-reset-button"),em=document.getElementById("show-modal-button"),eE=document.querySelector(".add-modal"),ey=document.getElementById("submit-recipe"),eC=document.getElementById("home-button"),ev=document.getElementById("favourite-button"),eO=document.getElementById("signOutBttn"),eI=document.querySelector(".recipe-list"),e_=document.getElementById("chat-history"),eT=document.getElementById("chat-input"),eb=document.getElementById("send-btn"),ew=document.getElementById("close-chat-btn"),eS=document.querySelector(".chatbot-buttons"),eR=document.getElementById("chatbot-container");let eA=[],eN=[];async function eM(){eL(await (0,el.loadRecipes)())}function eL(e){eI.innerHTML="",e.forEach(e=>{let t=document.createElement("div");t.classList.add("recipe-card"),t.innerHTML=`
        ...
        `;let n=t.querySelector(".edit-btn"),s=t.querySelector(".delete-btn");n.addEventListener("click",()=>{document.getElementById("recipe-title").value=e.title,document.getElementById("recipe-description").value=e.description,document.getElementById("recipe-favourite").checked=e.favourite;let t=[...e.steps],n=[...e.tags];eu.innerHTML=t.map(e=>`<li>${e}</li>`).join(""),ep.textContent=n.join(", "),eE.style.display="flex",em.style.backgroundColor="#f49cbb"}),s.addEventListener("click",async()=>{await (0,el.deleteRecipe)(e.id),eM()}),eI.appendChild(t)})}async function ex(){e=new er(await (0,el.getApiKey)()).getGenerativeModel({model:"gemini-1.5-flash"})}async function eD(t){try{let n=await e.generateContent(t),s=n.candidates[0]?.content.parts[0]?.text||"AI Error: No response.";ek(s)}catch(e){ek("AI Error: Unable to process request."),console.error(e)}}function ek(e){let t=document.createElement("div");t.textContent=e,t.className="history",e_.appendChild(t),eT.value=""}async function eH(e,t){let n={title:e,description:t,steps:[],tags:[],favourite:!1,createdAt:new Date().toISOString()};await (0,el.addRecipe)(n),eM()}async function e$(e){let t=!1;for(let n of(await (0,el.loadRecipes)()))if(n.title.toLowerCase()===e.toLowerCase()){await (0,el.deleteRecipe)(n.id),t=!0;break}t?(ek(`Recipe "${e}" deleted.`),eM()):ek(`Recipe "${e}" not found.`)}JSON.parse(localStorage.getItem("email"))||(window.location.href="index.html"),eO.addEventListener("click",function(){localStorage.removeItem("email"),window.location.href="index.html"}),document.addEventListener("DOMContentLoaded",async()=>{(0,el.loadRecipes)(),await eM(),await ex(),eb.addEventListener("click",async()=>{let e=eT.value.trim().toLowerCase();e?!function(e){if(e.startsWith("add recipe")){let t=e.replace("add recipe","").trim().split(";");if(t.length<2)return ek("Please specify a title and description using ';'. Example: 'add recipe Pancakes; A delicious breakfast.'"),!0;let n=t[0].trim(),s=t[1].trim();return n&&s?(eH(n,s),ek(`Recipe "${n}" added successfully!`)):ek("Invalid recipe format. Please provide both a title and description."),!0}if(e.startsWith("delete recipe")){let t=e.replace("delete recipe","").trim();return t?confirm(`Are you sure you want to delete "${t}"?`)&&e$(t):ek("Please specify the recipe title to delete."),!0}return!1}(e)&&eD(e):ek("Please enter a prompt")}),ed.addEventListener("click",()=>{let e=ec.value.trim();if(""!==e){eN.push(e);let t=document.createElement("li");t.textContent=e,eu.appendChild(t),ec.value=""}}),ef.addEventListener("click",()=>{let e=eh.value.trim();""!==e&&(eA.includes(e)||eA.push(e),ep.textContent="Tags: "+eA.join(", "),eh.value="")}),eg.addEventListener("click",()=>{document.querySelectorAll(".recipe-input").forEach(e=>e.value=""),document.getElementById("recipe-favourite").checked=!1,eu.innerHTML="",eA.length=0,ep.textContent=""}),ey.addEventListener("click",async()=>{let e=document.getElementById("recipe-title").value.trim(),t=document.getElementById("recipe-description").value.trim(),n=document.getElementById("recipe-favourite").checked;if(e&&t){let s={title:e,description:t,steps:[...eN],tags:[...eA],favourite:n,createdAt:new Date().toISOString()};await (0,el.addRecipe)(s),eg.click(),eE.style.display="none",em.style.backgroundColor="#f4f4f4",eA=[],eN=[]}eM()}),em.addEventListener("click",()=>{"none"===eE.style.display||""===eE.style.display?(eE.style.display="flex",em.style.backgroundColor="#f49cbb"):(eE.style.display="none",em.style.backgroundColor="#f4f4f4")}),eC.addEventListener("click",()=>{eE.style.display="none",em.style.backgroundColor="#f4f4f4",eM()}),ev.addEventListener("click",async()=>{eL((await (0,el.loadRecipes)()).filter(e=>e.favourite))}),e_.style.display="none",eT.style.display="none",eb.style.display="none",ew.innerText="AI Chat",eS.style.justifyContent="center",eR.style.width="auto";let e=!1;ew.addEventListener("click",()=>{let t="none"===e_.style.display;eR.style.width=t?"300px":"auto",e_.classList.toggle("hidden"),eT.classList.toggle("hidden"),eb.classList.toggle("hidden"),ew.innerText=t?"Close":"AI Chat",eS.style.justifyContent=t?"space-between":"center",t&&!e&&(ek('Welcome to the AI Chat! Commands:\n- **"add recipe Title; Description"** → Add a recipe.\n- **"delete recipe Title"** → Remove a recipe.'),e=!0)}),await eM()});
//# sourceMappingURL=recipes.a64c2012.js.map
