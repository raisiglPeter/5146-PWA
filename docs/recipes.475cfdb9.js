 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class er{constructor(e){this.apiKey=e}getGenerativeModel(e,t){if(!e.model)throw new N("Must provide a model name. Example: genai.getGenerativeModel({ model: 'my-model-name' })");return new ea(this.apiKey,e,t)}getGenerativeModelFromCachedContent(e,t,n){if(!e.name)throw new L("Cached content must contain a `name` field.");if(!e.model)throw new L("Cached content must contain a `model` field.");for(let n of["model","systemInstruction"])if((null==t?void 0:t[n])&&e[n]&&(null==t?void 0:t[n])!==e[n]){if("model"===n&&(t.model.startsWith("models/")?t.model.replace("models/",""):t.model)===(e.model.startsWith("models/")?e.model.replace("models/",""):e.model))continue;throw new L(`Different value for "${n}" specified in modelParams (${t[n]}) and cachedContent (${e[n]})`)}let s=Object.assign(Object.assign({},t),{model:e.model,tools:e.tools,toolConfig:e.toolConfig,systemInstruction:e.systemInstruction,cachedContent:e});return new ea(this.apiKey,s,n)}}var el=S("47Mwn");const ec=document.getElementById("recipe-steps"),ed=document.querySelector(".recipe-step-button"),eu=document.querySelector(".steps-preview ol"),eh=document.getElementById("recipe-tags"),ef=document.querySelector(".recipe-tag-button"),ep=document.querySelector(".tags-preview p"),em=document.getElementById("add-modal-reset-button"),eg=document.getElementById("show-modal-button"),eE=document.querySelector(".add-modal"),ey=document.getElementById("submit-recipe"),eC=document.getElementById("home-button"),ev=document.getElementById("favourite-button"),eO=document.getElementById("signOutBttn"),eI=document.querySelector(".recipe-list"),e_=document.getElementById("chat-history"),eb=document.getElementById("chat-input"),eT=document.getElementById("send-btn"),ew=document.getElementById("close-chat-btn"),eA=document.querySelector(".chatbot-buttons"),eS=document.getElementById("chatbot-container"),eR=document.getElementById("chat-input-label");let eN=[],eM=[];async function ex(){eD(await (0,el.loadRecipes)())}function eL(e){eE.style.display=e?"flex":"none",eg.style.backgroundColor=e?"#f49cbb":"#f4f4f4",eg.innerText=e?"Close":"Add",eE.setAttribute("aria-hidden",e?"false":"true")}function eD(e){eI.innerHTML="",e.forEach(e=>{let t=document.createElement("li");t.classList.add("recipe-card"),t.innerHTML=`
        ...
        `;let n=t.querySelector(".edit-btn"),s=t.querySelector(".delete-btn");n.addEventListener("click",()=>{document.getElementById("recipe-title").value=e.title,document.getElementById("recipe-description").value=e.description,document.getElementById("recipe-favourite").checked=e.favourite;let t=[...e.steps],n=[...e.tags];eu.innerHTML=t.map(e=>`<li>${e}</li>`).join(""),ep.textContent=n.join(", "),eL(!0)}),s.addEventListener("click",async()=>{await (0,el.deleteRecipe)(e.id),ex()}),eI.appendChild(t)})}async function ek(){e=new er(await (0,el.getApiKey)()).getGenerativeModel({model:"gemini-1.5-flash"})}async function eB(t){try{let n=await e.generateContent(t),s=n.candidates[0]?.content.parts[0]?.text||"AI Error: No response.";eH(s)}catch(e){eH("AI Error: Unable to process request."),console.error(e)}}function eH(e){let t=document.createElement("div");t.textContent=e,t.className="history",e_.appendChild(t),eb.value=""}async function e$(e,t){let n={title:e,description:t,steps:[],tags:[],favourite:!1,createdAt:new Date().toISOString()};await (0,el.addRecipe)(n),ex()}async function eU(e){let t=!1;for(let n of(await (0,el.loadRecipes)()))if(n.title.toLowerCase()===e.toLowerCase()){await (0,el.deleteRecipe)(n.id),t=!0;break}t?(eH(`Recipe "${e}" deleted.`),ex()):eH(`Recipe "${e}" not found.`)}JSON.parse(localStorage.getItem("email"))||(window.location.href="index.html"),eO.addEventListener("click",function(){localStorage.removeItem("email"),window.location.href="index.html"}),document.addEventListener("DOMContentLoaded",async()=>{(0,el.loadRecipes)(),await ex(),await ek(),eT.addEventListener("click",async()=>{let e=eb.value.trim().toLowerCase();e?!function(e){if(e.startsWith("add recipe")){let t=e.replace("add recipe","").trim().split(";");if(t.length<2)return eH("Please specify a title and description using ';'. Example: 'add recipe Pancakes; A delicious breakfast.'"),!0;let n=t[0].trim(),s=t[1].trim();return n&&s?(e$(n,s),eH(`Recipe "${n}" added successfully!`)):eH("Invalid recipe format. Please provide both a title and description."),!0}if(e.startsWith("delete recipe")){let t=e.replace("delete recipe","").trim();return t?confirm(`Are you sure you want to delete "${t}"?`)&&eU(t):eH("Please specify the recipe title to delete."),!0}return!1}(e)&&eB(e):eH("Please enter a prompt")}),ed.addEventListener("click",()=>{let e=ec.value.trim();if(""!==e){eM.push(e);let t=document.createElement("li");t.textContent=e,eu.appendChild(t),ec.value=""}}),ef.addEventListener("click",()=>{let e=eh.value.trim();""!==e&&(eN.includes(e)||eN.push(e),ep.textContent="Tags: "+eN.join(", "),eh.value="")}),em.addEventListener("click",()=>{document.querySelectorAll(".recipe-input").forEach(e=>e.value=""),document.getElementById("recipe-favourite").checked=!1,eu.innerHTML="",eN=[],eM=[],ep.textContent=""}),ey.addEventListener("click",async()=>{let e=document.getElementById("recipe-title").value.trim(),t=document.getElementById("recipe-description").value.trim(),n=document.getElementById("recipe-favourite").checked;if(e&&t){let s={title:e,description:t,steps:[...eM],tags:[...eN],favourite:n,createdAt:new Date().toISOString()};await (0,el.addRecipe)(s),em.click(),eL(!1)}}),eg.addEventListener("click",()=>{console.log(eg),console.log("add modal button works");let e="true"===eE.getAttribute("aria-hidden");console.log(e),console.log("add modal display=",eE.style.display),eL(!e),console.log("add modal display=",eE.style.display),console.log(e)}),eC.addEventListener("click",()=>{eE.style.display="none",eg.style.backgroundColor="#f4f4f4",ex()}),ev.addEventListener("click",async()=>{eD((await (0,el.loadRecipes)()).filter(e=>e.favourite))}),ev.addEventListener("keydown",e=>{if("Enter"===e.key){let e=document.activeElement;"checkbox"===e.type&&(e.checked=!e.checked)}}),e_.style.display="none",eb.style.display="none",eR.style.display="none",eT.style.display="none",ew.innerText="AI Chat",eA.style.justifyContent="center",eS.style.width="auto";let e=!1;ew.addEventListener("click",()=>{let t="none"===e_.style.display;eS.style.width=t?"300px":"auto",e_.style.display=t?"block":"none",eR.style.display=t?"block":"none",eb.style.display=t?"block":"none",eT.style.display=t?"block":"none",ew.innerText=t?"Close":"AI Chat",eA.style.justifyContent=t?"space-between":"center",t&&!e&&(eH('Welcome to the AI Chat! Available commands:\n<br/>- add recipe Title; Description"** → Add a recipe.\n<br/>- delete recipe Title"** → Remove a recipe.'),e=!0)}),await ex()});
//# sourceMappingURL=recipes.475cfdb9.js.map
