let e;var t,n,s,i,o,a,r,l,c,d,u,h,p,f,m,g,E,y,C,v,I,O,_,b,A=globalThis,T={},w={},S=A.parcelRequire94c2;null==S&&((S=function(e){if(e in T)return T[e].exports;if(e in w){var t=w[e];delete w[e];var n={id:e,exports:{}};return T[e]=n,t.call(n.exports,n,n.exports),n.exports}var s=Error("Cannot find module '"+e+"'");throw s.code="MODULE_NOT_FOUND",s}).register=function(e,t){w[e]=t},A.parcelRequire94c2=S),S.register,(t=p||(p={})).STRING="string",t.NUMBER="number",t.INTEGER="integer",t.BOOLEAN="boolean",t.ARRAY="array",t.OBJECT="object",(n=f||(f={})).LANGUAGE_UNSPECIFIED="language_unspecified",n.PYTHON="python",(s=m||(m={})).OUTCOME_UNSPECIFIED="outcome_unspecified",s.OUTCOME_OK="outcome_ok",s.OUTCOME_FAILED="outcome_failed",s.OUTCOME_DEADLINE_EXCEEDED="outcome_deadline_exceeded";/**
 * @license
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */async function ei(e,t,n,s){return(await H(t,b.EMBED_CONTENT,e,!1,JSON.stringify(n),s)).json()}async function eo(e,t,n,s){let i=n.requests.map(e=>Object.assign(Object.assign({},e),{model:t}));return(await H(t,b.BATCH_EMBED_CONTENTS,e,!1,JSON.stringify({requests:i}),s)).json()}/**
 * @license
 * Copyright 2024 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 */class er{constructor(e){this.apiKey=e}getGenerativeModel(e,t){if(!e.model)throw new N("Must provide a model name. Example: genai.getGenerativeModel({ model: 'my-model-name' })");return new ea(this.apiKey,e,t)}getGenerativeModelFromCachedContent(e,t,n){if(!e.name)throw new L("Cached content must contain a `name` field.");if(!e.model)throw new L("Cached content must contain a `model` field.");for(let n of["model","systemInstruction"])if((null==t?void 0:t[n])&&e[n]&&(null==t?void 0:t[n])!==e[n]){if("model"===n&&(t.model.startsWith("models/")?t.model.replace("models/",""):t.model)===(e.model.startsWith("models/")?e.model.replace("models/",""):e.model))continue;throw new L(`Different value for "${n}" specified in modelParams (${t[n]}) and cachedContent (${e[n]})`)}let s=Object.assign(Object.assign({},t),{model:e.model,tools:e.tools,toolConfig:e.toolConfig,systemInstruction:e.systemInstruction,cachedContent:e});return new ea(this.apiKey,s,n)}}var el=S("47Mwn");const ec=document.querySelector(".add-modal"),ed=document.getElementById("home-button"),eu=document.getElementById("favourite-button"),eh=document.getElementById("signOutBttn"),ep=document.getElementById("show-modal-button"),ef=document.getElementById("recipe-steps"),em=document.getElementById("recipe-tags"),eg=document.querySelector(".recipe-step-button"),eE=document.querySelector(".recipe-tag-button"),ey=document.querySelector(".steps-preview ol"),eC=document.querySelector(".tags-preview p"),ev=document.getElementById("add-modal-reset-button"),eI=document.getElementById("submit-recipe"),eO=document.querySelector(".recipe-list"),e_=document.getElementById("chat-history"),eb=document.getElementById("chat-input"),eA=document.getElementById("send-btn"),eT=document.getElementById("close-chat-btn"),ew=document.querySelector(".chatbot-buttons"),eS=document.getElementById("chatbot-container"),eR=document.getElementById("chat-input-label");let eN=[],ex=[];async function eM(){eD(await (0,el.loadRecipes)())}function eL(e){ec.style.display=e?"flex":"none",ep.style.backgroundColor=e?"#f49cbb":"#f4f4f4",ep.innerText=e?"Close":"Add",ec.setAttribute("aria-hidden",e?"false":"true")}function eD(e){eO.innerHTML="",e.forEach(e=>{let t=document.createElement("li");t.classList.add("recipe-card"),t.innerHTML=`
        ...
        `;let n=t.querySelector(".edit-btn"),s=t.querySelector(".delete-btn");n.addEventListener("click",()=>{document.getElementById("recipe-title").value=e.title,document.getElementById("recipe-description").value=e.description,document.getElementById("recipe-favourite").checked=e.favourite;let t=[...e.steps],n=[...e.tags];ey.innerHTML=t.map(e=>`<li>${e}</li>`).join(""),eC.textContent=n.join(", "),eL(!0)}),s.addEventListener("click",async()=>{await (0,el.deleteRecipe)(e.id),eM()}),eO.appendChild(t)})}async function ek(){e=new er(await (0,el.getApiKey)()).getGenerativeModel({model:"gemini-1.5-flash"})}async function eB(t){try{if(console.log("askChatBot() called with request:",t),!e){eH("AI Error: Model not initialized."),console.error("Error: Model instance is undefined or null.");return}let n=await e.generateContent(t);if(console.log("Full API response:",n),!n||!n.candidates||0===n.candidates.length){eH("AI Error: No response from AI."),console.error("Error: Empty or invalid AI response:",n);return}let s=n.candidates[0]?.content?.parts?.[0]?.text;if(!s){eH("AI Error: No valid text response."),console.error("Error: AI response missing text:",n);return}console.log("Extracted AI response:",s),eH(s)}catch(e){eH("AI Error: Unable to process request."),console.error("AI Request Error:",e)}}function eH(e){let t=document.createElement("div");t.textContent=e,t.className="history",e_.appendChild(t),eb.value=""}async function e$(e,t){let n={title:e,description:t,steps:[],tags:[],favourite:!1,createdAt:new Date().toISOString()};await (0,el.addRecipe)(n),ej("Added recipe: "+n.title),eM()}async function eP(e){let t=!1;for(let n of(await (0,el.loadRecipes)()))if(n.title.toLowerCase()===e.toLowerCase()){await (0,el.deleteRecipe)(n.id),t=!0;break}t?(eH(`Recipe "${e}" deleted.`),eM()):eH(`Recipe "${e}" not found.`)}JSON.parse(localStorage.getItem("email"))||(window.location.href="index.html"),eh.addEventListener("click",function(){localStorage.removeItem("email"),window.location.href="index.html"});const eU=document.getElementById("notification");function ej(e){eU.textContent=e,eU.style.visibility="visible",setTimeout(()=>{eU.style.visibility="hidden",eU.textContent=""},3e3)}document.addEventListener("DOMContentLoaded",async()=>{(0,el.loadRecipes)(),ep.focus(),await eM(),await ek(),eA.addEventListener("click",async()=>{let e=eb.value.trim();if(!e){eH("Please enter a prompt");return}e&&!function(e){if(e.startsWith("add recipe")){let t=e.replace("add recipe","").trim().split(";");if(t.length<2)return eH("Please specify a title and description using ';'. Example: 'add recipe Pancakes; A delicious breakfast.'"),!0;let n=t[0].trim(),s=t[1].trim();return n&&s?(e$(n,s),eH(`Recipe "${n}" added successfully!`)):eH("Invalid recipe format. Please provide both a title and description."),!0}if(e.startsWith("delete recipe")){let t=e.replace("delete recipe","").trim();return t?confirm(`Are you sure you want to delete "${t}"?`)&&eP(t):eH("Please specify the recipe title to delete."),!0}return!1}(e)&&eB(e)}),eg.addEventListener("click",()=>{let e=ef.value.trim();if(""!==e){ex.push(e);let t=document.createElement("li");t.textContent=e,ey.appendChild(t),ef.value=""}}),eE.addEventListener("click",()=>{let e=em.value.trim();""!==e&&(eN.includes(e)||eN.push(e),eC.textContent="Tags: "+eN.join(", "),em.value="")}),ev.addEventListener("click",()=>{document.querySelectorAll(".recipe-input").forEach(e=>e.value=""),document.getElementById("recipe-favourite").checked=!1,ey.innerHTML="",eN=[],ex=[],eC.textContent=""}),eI.addEventListener("click",async()=>{let e=document.getElementById("recipe-title"),t=document.getElementById("recipe-description"),n=document.querySelector(".input-validation-message"),s=e.value.trim(),i=t.value.trim(),o=document.getElementById("recipe-favourite").checked;if(s&&i){let a={title:s,description:i,steps:[...ex],tags:[...eN],favourite:o,createdAt:new Date().toISOString()};await (0,el.addRecipe)(a),n.style.display="none",e.classList.remove("recipe-input-highlight"),t.classList.remove("recipe-input-highlight"),ev.click(),ej("Added recipe: "+a.title),eL(!1),eM()}else n.style.display="flex",s?e.classList.remove("recipe-input-highlight"):e.classList.add("recipe-input-highlight"),i?t.classList.remove("recipe-input-highlight"):t.classList.add("recipe-input-highlight")}),ep.addEventListener("click",()=>{eL("flex"!==ec.style.display)}),ed.addEventListener("click",()=>{eM(),eL(!1)}),eu.addEventListener("click",async()=>{eD((await (0,el.loadRecipes)()).filter(e=>e.favourite)),eL(!1)}),eu.addEventListener("keydown",e=>{if("Enter"===e.key){let e=document.activeElement;"checkbox"===e.type&&(e.checked=!e.checked)}}),e_.style.display="none",eb.style.display="none",eR.style.display="none",eA.style.display="none",eT.innerText="AI Chat",ew.style.justifyContent="center",eS.style.width="auto";let e=!1;eT.addEventListener("click",()=>{let t="none"===e_.style.display;eS.style.width=t?"300px":"auto",e_.style.display=t?"block":"none",eR.style.display=t?"block":"none",eb.style.display=t?"block":"none",eA.style.display=t?"block":"none",eT.innerText=t?"Close":"AI Chat",ew.style.justifyContent=t?"space-between":"center",t&&!e&&(eH(`Welcome to the AI Chat! Commands: add recipe "Title; Description" will add recipe.
- delete recipe "Title" will remove a recipe.`),e=!0)}),await eM()});
//# sourceMappingURL=recipes.ac2ec7ea.js.map
