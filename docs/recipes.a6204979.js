let e;var t,n,i,s,o,a,r,l,c,d,u,h,f,p,m,g,E,y,C,v,O,I,_,b,T=globalThis,w={},A={},S=T.parcelRequire94c2;null==S&&((S=function(e){if(e in w)return w[e].exports;if(e in A){var t=A[e];delete A[e];var n={id:e,exports:{}};return w[e]=n,t.call(n.exports,n,n.exports),n.exports}var i=Error("Cannot find module '"+e+"'");throw i.code="MODULE_NOT_FOUND",i}).register=function(e,t){A[e]=t},T.parcelRequire94c2=S),S.register,(t=f||(f={})).STRING="string",t.NUMBER="number",t.INTEGER="integer",t.BOOLEAN="boolean",t.ARRAY="array",t.OBJECT="object",(n=p||(p={})).LANGUAGE_UNSPECIFIED="language_unspecified",n.PYTHON="python",(i=m||(m={})).OUTCOME_UNSPECIFIED="outcome_unspecified",i.OUTCOME_OK="outcome_ok",i.OUTCOME_FAILED="outcome_failed",i.OUTCOME_DEADLINE_EXCEEDED="outcome_deadline_exceeded";/**
 * @license
 ...
 */class er{constructor(e){this.apiKey=e}getGenerativeModel(e,t){if(!e.model)throw new N("Must provide a model name. Example: genai.getGenerativeModel({ model: 'my-model-name' })");return new ea(this.apiKey,e,t)}getGenerativeModelFromCachedContent(e,t,n){if(!e.name)throw new x("Cached content must contain a `name` field.");if(!e.model)throw new x("Cached content must contain a `model` field.");for(let n of["model","systemInstruction"])if((null==t?void 0:t[n])&&e[n]&&(null==t?void 0:t[n])!==e[n]){if("model"===n&&(t.model.startsWith("models/")?t.model.replace("models/",""):t.model)===(e.model.startsWith("models/")?e.model.replace("models/",""):e.model))continue;throw new x(`Different value for "${n}" specified in modelParams (${t[n]}) and cachedContent (${e[n]})`)}let i=Object.assign(Object.assign({},t),{model:e.model,tools:e.tools,toolConfig:e.toolConfig,systemInstruction:e.systemInstruction,cachedContent:e});return new ea(this.apiKey,i,n)}}var el=S("47Mwn");const ec=document.querySelector(".add-modal"),ed=document.getElementById("home-button"),eu=document.getElementById("favourite-button"),eh=document.getElementById("signOutBttn"),ef=document.getElementById("show-modal-button"),ep=document.getElementById("recipe-steps"),em=document.getElementById("recipe-tags"),eg=document.querySelector(".recipe-step-button"),eE=document.querySelector(".recipe-tag-button"),ey=document.querySelector(".steps-preview ol"),eC=document.querySelector(".tags-preview p"),ev=document.getElementById("add-modal-reset-button"),eO=document.getElementById("submit-recipe"),eI=document.querySelector(".recipe-list"),e_=document.getElementById("chat-history"),eb=document.getElementById("chat-input"),eT=document.getElementById("send-btn"),ew=document.getElementById("close-chat-btn"),eA=document.querySelector(".chatbot-buttons"),eS=document.getElementById("chatbot-container"),eR=document.getElementById("chat-input-label");let eN=[],eM=[];async function eL(){eD(await (0,el.loadRecipes)())}function ex(e){ec.style.display=e?"flex":"none",ef.style.backgroundColor=e?"#f49cbb":"#f4f4f4",ef.innerText=e?"Close":"Add",ec.setAttribute("aria-hidden",e?"false":"true")}function eD(e){eI.innerHTML="",e.forEach(e=>{let t=document.createElement("li");t.classList.add("recipe-card"),t.innerHTML=`
        ...
        `;let n=t.querySelector(".edit-btn"),i=t.querySelector(".delete-btn");n.addEventListener("click",()=>{document.getElementById("recipe-title").value=e.title,document.getElementById("recipe-description").value=e.description,document.getElementById("recipe-favourite").checked=e.favourite;let t=[...e.steps],n=[...e.tags];ey.innerHTML=t.map(e=>`<li>${e}</li>`).join(""),eC.textContent=n.join(", "),ex(!0)}),i.addEventListener("click",async()=>{await (0,el.deleteRecipe)(e.id),eL()}),eI.appendChild(t)})}async function ek(){e=new er(await (0,el.getApiKey)()).getGenerativeModel({model:"gemini-1.5-flash"})}async function eB(t){try{if(!e){eH("AI Error: Model not initialized.");return}let n=await e.generateContent(t);if(!n||!n.candidates||0===n.candidates.length){eH("AI Error: No response from AI.");return}let i=n?.candidates?.[0]?.content?.parts?.[0]?.text;if(!i){eH("AI Error: No valid response. Try reloading the page.");return}eH(i)}catch(e){eH("AI Error: Unable to process request.")}}function eH(e){let t=document.createElement("div");t.textContent=e,t.className="history",e_.appendChild(t),eb.value=""}async function e$(e,t){let n={title:e,description:t,steps:[],tags:[],favourite:!1,createdAt:new Date().toISOString()};await (0,el.addRecipe)(n),ej("Added recipe: "+n.title),eL()}async function eU(e){let t=!1;for(let n of(await (0,el.loadRecipes)()))if(n.title.toLowerCase()===e.toLowerCase()){await (0,el.deleteRecipe)(n.id),t=!0;break}t?(eH(`Recipe "${e}" deleted.`),eL()):eH(`Recipe "${e}" not found.`)}JSON.parse(localStorage.getItem("email"))||(window.location.href="index.html"),eh.addEventListener("click",function(){localStorage.removeItem("email"),window.location.href="index.html"});const eP=document.getElementById("notification");function ej(e){eP.textContent=e,eP.style.visibility="visible",setTimeout(()=>{eP.style.visibility="hidden",eP.textContent=""},3e3)}document.addEventListener("DOMContentLoaded",async()=>{(0,el.loadRecipes)(),ef.focus(),await eL(),await ek(),eT.addEventListener("click",async()=>{let e=eb.value.trim();if(!e){eH("Please enter a prompt");return}e&&!function(e){if(e.startsWith("add recipe")){let t=e.replace("add recipe","").trim().split(";");if(t.length<2)return eH("Please specify a title and description using ';'. Example: 'add recipe Pancakes; A delicious breakfast.'"),!0;let n=t[0].trim(),i=t[1].trim();return n&&i?(e$(n,i),eH(`Recipe "${n}" added successfully!`)):eH("Invalid recipe format. Please provide both a title and description."),!0}if(e.startsWith("delete recipe")){let t=e.replace("delete recipe","").trim();return t?confirm(`Are you sure you want to delete "${t}"?`)&&eU(t):eH("Please specify the recipe title to delete."),!0}return!1}(e)&&eB(e)}),eg.addEventListener("click",()=>{let e=ep.value.trim();if(""!==e){eM.push(e);let t=document.createElement("li");t.textContent=e,ey.appendChild(t),ep.value=""}}),eE.addEventListener("click",()=>{let e=em.value.trim();""!==e&&(eN.includes(e)||eN.push(e),eC.textContent="Tags: "+eN.join(", "),em.value="")}),ev.addEventListener("click",()=>{document.querySelectorAll(".recipe-input").forEach(e=>e.value=""),document.getElementById("recipe-favourite").checked=!1,ey.innerHTML="",eN=[],eM=[],eC.textContent=""}),eO.addEventListener("click",async()=>{let e=document.getElementById("recipe-title"),t=document.getElementById("recipe-description"),n=document.querySelector(".input-validation-message"),i=e.value.trim(),s=t.value.trim(),o=document.getElementById("recipe-favourite").checked;if(i&&s){let a={title:i,description:s,steps:[...eM],tags:[...eN],favourite:o,createdAt:new Date().toISOString()};await (0,el.addRecipe)(a),n.style.display="none",e.classList.remove("recipe-input-highlight"),t.classList.remove("recipe-input-highlight"),ev.click(),ej("Added recipe: "+a.title),ex(!1),eL()}else n.style.display="flex",i?e.classList.remove("recipe-input-highlight"):e.classList.add("recipe-input-highlight"),s?t.classList.remove("recipe-input-highlight"):t.classList.add("recipe-input-highlight")}),ef.addEventListener("click",()=>{ex("flex"!==ec.style.display)}),ed.addEventListener("click",()=>{eL(),ex(!1)}),eu.addEventListener("click",async()=>{eD((await (0,el.loadRecipes)()).filter(e=>e.favourite)),ex(!1)}),eu.addEventListener("keydown",e=>{if("Enter"===e.key){let e=document.activeElement;"checkbox"===e.type&&(e.checked=!e.checked)}}),e_.style.display="none",eb.style.display="none",eR.style.display="none",eT.style.display="none",ew.innerText="AI Chat",eA.style.justifyContent="center",eS.style.width="auto";let e=!1;ew.addEventListener("click",()=>{let t="none"===e_.style.display;eS.style.width=t?"300px":"auto",e_.style.display=t?"block":"none",eR.style.display=t?"block":"none",eb.style.display=t?"block":"none",eT.style.display=t?"block":"none",ew.innerText=t?"Close":"AI Chat",eA.style.justifyContent=t?"space-between":"center",t&&!e&&(eH(`Welcome to the AI Chat! Commands: add recipe "Title; Description" will add recipe.
- delete recipe "Title" will remove a recipe.`),e=!0)}),await eL()});
//# sourceMappingURL=recipes.a6204979.js.map
